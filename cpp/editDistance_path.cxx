...
inline float editCost(int n1, int p1, int n2, int p2, std::vector<float> &nodes1, std::vector<float> &nodes2){
    if(n1<0){
        float b1 = nodes2[n2];
        float d1 = nodes2[p2];
...
float editDistance_path(std::vector<float> &nodes1,
    ...
    for(int i=0; i<nn1; i++){
        for(int l=1; l<=predecessors1[i].size(); l++){
            ...
            memT[curr1+l*dim2+nn2*dim3+0*dim4] = editCost(curr1,parent1,-1,-1,nodes1,nodes2);
    ...
    for(int j=0; j<nn2; j++){
        for(int l=1; l<=predecessors2[j].size(); l++){
            ...
            //-----------------------------------------------------------------------
            // Delete curr path and full subtree rooted in path
            memT[nn1+0*dim2+curr2*dim3+l*dim4] = editCost(-1,-1,curr2,parent2,nodes1,nodes2);
            ...
            for(int l1=1; l1<=predecessors1[i].size(); l1++){
                for(int l2=1; l2<=predecessors2[j].size(); l2++){
                    ...
                    if(topo1[curr1].size()==0 and topo2[curr2].size()==0){
                        memT[curr1+l1*dim2+curr2*dim3+l2*dim4] = editCost(curr1,parent1,curr2,parent2,nodes1,nodes2);
                    }
                    //---------------------------------------------------------------------------
                    ...
                    // If both trees have more than one branch, try all decompositions of both trees
                        ...
                        if(topo1[curr1].size()==2 && topo2[curr2].size()==2){
                            ...
                            d = std::min(d,memT[child11+1*dim2+child21*dim3+1*dim4] + memT[child12+1*dim2+child22*dim3+1*dim4] + editCost(curr1,parent1,curr2,parent2,nodes1,nodes2));
                            d = std::min(d,memT[child11+1*dim2+child22*dim3+1*dim4] + memT[child12+1*dim2+child21*dim3+1*dim4] + editCost(curr1,parent1,curr2,parent2,nodes1,nodes2));
                        }
                        else{
                            float d_ = editCost(curr1,parent1,curr2,parent2,nodes1,nodes2);
